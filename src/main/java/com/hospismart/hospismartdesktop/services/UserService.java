...
public class UserService {
    ...
    public boolean ajouter(User user) throws SQLException {
        String query = "INSERT INTO user (nom, roles, password, prenom, email, telephone, type, specialite, service_entity_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        // Ensure the 'type' field is set. Extract "patient" from "ROLE_PATIENT" if necessary.
        String simpleType = "patient";
        if (t != null && !t.isEmpty()) {
            if (t.toUpperCase().contains("ADMIN")) simpleType = "admin";
            else if (t.toUpperCase().contains("MEDECIN") || t.toUpperCase().contains("PRATICIEN")) simpleType = "medecin";
        }
        pst.setString(7, simpleType);
        pst.setString(8, user.getSpecialite());
        if (user.getServiceId() > 0) {
            pst.setInt(9, user.getServiceId());
        } else {
    ...
    public boolean modifier(User user) {
        String query = "UPDATE user SET nom=?, roles=?, password=?, prenom=?, email=?, telephone=?, type=?, specialite=?, service_entity_id=? WHERE id=?";
        try {
            PreparedStatement pst = cnx.prepareStatement(query);
            pst.setString(1, user.getNom());
            String t = user.getType();
